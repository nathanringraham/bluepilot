...
def _infer_mhp(slice_size: int, product_output_shape: int, max_in_n: int = 16, max_out_n: int = 6) -> tuple[int, int]:
  for out_n in range(max_out_n + 1):
    per_hypothesis = 2 * product_output_shape + out_n
    if per_hypothesis > 0 and slice_size % per_hypothesis == 0:
...
class Parser:
  ...
  def parse_mdn(self, name, outs, out_shape, in_N=0, out_N=0):
    ...
    if out_N > 1 or (in_N > 1 and out_N == 0):
      selections = out_N if out_N > 1 else in_N
      final_shape = tuple([raw.shape[0], selections] + list(out_shape))
    else:
